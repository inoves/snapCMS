...
class ActiveRecord {
  ...
  function __get($name) {
    if (array_key_exists($name, $this->attributes))
	  	$to_return = stripslashes($this->attributes[$name]);
    elseif (array_key_exists($name, $this->associations))
      $to_return = $this->associations[$name]->get($this);
    ...
    elseif (preg_match('/^(.+?)_ids$/', $name, $matches)) {
      ...
      if ($this->associations[$assoc_name] instanceof HasMany)
        $to_return = $this->associations[$assoc_name]->get_ids($this);
    }
    return $to_return;
  ...
  */
  ...
  static function query_obj($query, $classname='SnapClass'){
		$rows = ActiveRecord::query($query);
		$new_rows = array();
	    if (!empty($rows)) {
	        //$data = new SnapClass();
	        foreach ($rows as $k => $row){
				$new_rows[$k] = new $classname();
				foreach($row as $akey => $aval){
	            	$new_rows[$k]->$akey = $aval;
				}
	        }
	        return $new_rows;
	    }
